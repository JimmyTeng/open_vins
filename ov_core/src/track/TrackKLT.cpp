...
void TrackKLT::feed_monocular(const CameraData &message, size_t msg_id) {
  ...
  // This also handles, the tracking initalization on the first call to this
  // extractor
  ...
  // If any of our mask is empty, that means we didn't have enough to do ransac,
  // so just return
  if (mask_ll.empty()) {
    std::lock_guard<std::mutex> lckv(mtx_last_vars);
    img_last[cam_id] = img;
  ...
  for (size_t i = 0; i < pts_left_new.size(); i++) {
    ...
    if ((int)mask.at<uint8_t>((int)pts_left_new.at(i).pt.y,
  ...
  // rT4)); PRINT_INFO("[TIME-KLT]: %.4f seconds for feature DB update (%d
  // features)\n", rtime_sec(rT4, rT5),
  //           (int)good_left.size());
  // PRINT_INFO("[TIME-KLT]: %.4f seconds for total\n", rtime_sec(rT1, rT5));
}

void TrackKLT::feed_stereo(const CameraData &message, size_t msg_id_left,
                           size_t msg_id_right) {
  // Lock this data feed for this camera
  ...
  if (mask_left.empty() || mask_left.rows != img_left.rows ||
      mask_left.cols != img_left.cols)
    mask_left = cv::Mat::zeros(img_left.rows, img_left.cols, CV_8UC1);
  if (mask_right.empty() || mask_right.rows != img_right.rows ||
      mask_right.cols != img_right.cols)
  ...
  if (pts_last[cam_id_left].empty() && pts_last[cam_id_right].empty()) {
    ...
    perform_detection_stereo(imgpyr_left, imgpyr_right, mask_left, mask_right,
                             cam_id_left, cam_id_right, good_left, good_right,
  ...
  perform_detection_stereo(
      img_pyramid_last[cam_id_left], img_pyramid_last[cam_id_right],
      img_mask_last[cam_id_left], img_mask_last[cam_id_right], cam_id_left,
      cam_id_right, pts_left_old, pts_right_old, ids_left_old, ids_right_old);
  rT3 = rtime_now();
  ...
  // TODO: maybe we should collect all tracks that are in both frames and make
  // they pass this? std::vector<uchar> mask_lr; perform_matching(imgpyr_left,
  // imgpyr_right, pts_left_new, pts_right_new, cam_id_left, cam_id_right,
  // mask_lr);
  rT5 = rtime_now();
  ...
  // If any of our masks are empty, that means we didn't have enough to do
  // ransac, so just return
  ...
  for (size_t i = 0; i < pts_left_new.size(); i++) {
    ...
    if (mask_ll[i] && found_right && mask_rr[index_right]) {
      // Ensure we do not have any bad KLT tracks (i.e., points are negative)
      if (pts_right_new.at(index_right).pt.x < 0 ||
          pts_right_new.at(index_right).pt.y < 0 ||
  ...
  for (size_t i = 0; i < pts_right_new.size(); i++) {
    ...
    // See if we have the same feature in the right
    bool added_already =
        (std::find(good_ids_right.begin(), good_ids_right.end(),
                   ids_right_old.at(i)) != good_ids_right.end());
    // If it has not already been added as a good feature, add it as a mono
    // track
  ...
  for (size_t i = 0; i < good_right.size(); i++) {
    ...
    database->update_feature(good_ids_right.at(i), message.timestamp,
...
void TrackKLT::perform_detection_monocular(const std::vector<cv::Mat> &img0pyr,
  ...
  cv::Mat mask0_use = mask0.empty()
                          ? cv::Mat::zeros(img0pyr.at(0).size(), CV_8UC1)
                          : mask0.clone();
  ...
  while (it0 != pts0.end()) {
    ...
    if (x_close < 0 || x_close >= size_close.width || y_close < 0 ||
        y_close >= size_close.height) {
      it0 = pts0.erase(it0);
      it1 = ids0.erase(it1);
      continue;
    }
    ...
    if (x_grid < 0 || x_grid >= size_grid.width || y_grid < 0 ||
        y_grid >= size_grid.height) {
      it0 = pts0.erase(it0);
      it1 = ids0.erase(it1);
      continue;
    }
  ...
  // This can be slow as this will recompute extractions for grid areas that we
  // have max features already std::vector<cv::KeyPoint> pts0_ext;
  // Grider_FAST::perform_griding(img0pyr.at(0), mask0_updated, pts0_ext,
  // num_features, grid_x, grid_y, threshold, true);

  // We also check a downsampled mask such that we don't extract in areas where
  // it is all masked!
  cv::Mat mask0_grid;
  if (!mask0_use.empty()) {
  ...
  // Create grids we need to extract from and then extract our features (use
  // fast with griding)
  int num_features_grid =
      (int)((double)num_features / (double)(grid_x * grid_y)) + 1;
  int num_features_grid_req =
      std::max(1, (int)(min_feat_percent * num_features_grid));
  ...
  for (auto &kpt : pts0_ext) {
    ...
    if (x_grid < 0 || x_grid >= size_close.width || y_grid < 0 ||
        y_grid >= size_close.height)
      continue;
...
void TrackKLT::perform_detection_stereo(
  ...
  while (it0 != pts0.end()) {
    ...
    int x_close = (int)(kpt.pt.x / (float)min_px_dist);
    int y_close = (int)(kpt.pt.y / (float)min_px_dist);
    if (x_close < 0 || x_close >= size_close0.width || y_close < 0 ||
        y_close >= size_close0.height) {
      it0 = pts0.erase(it0);
      it1 = ids0.erase(it1);
      continue;
    }
    ...
    if (x_grid < 0 || x_grid >= size_grid0.width || y_grid < 0 ||
        y_grid >= size_grid0.height) {
      it0 = pts0.erase(it0);
      it1 = ids0.erase(it1);
      continue;
    }
  ...
  // LEFT: we will also try to track them from this frame over to the right
  // frame LEFT: in the case that we have two features that are the same, then
  // we should merge them
  if (num_featsneeded_0 >
      std::min(20, (int)(min_feat_percent * num_features))) {
    // This is old extraction code that would extract from the whole image
    // This can be slow as this will recompute extractions for grid areas that
    // we have max features already std::vector<cv::KeyPoint> pts0_ext;
    // Grider_FAST::perform_griding(img0pyr.at(0), mask0_updated, pts0_ext,
    // num_features, grid_x, grid_y, threshold, true);

    // We also check a downsampled mask such that we don't extract in areas
    // where it is all masked!
    cv::Mat mask0_grid;
    if (!mask0_use.empty()) {
      cv::resize(mask0_use, mask0_grid, size_grid0, 0.0, 0.0,
                 cv::INTER_NEAREST);
    } else {
    ...
    for (int x = 0; x < grid_2d_grid0.cols; x++) {
      for (int y = 0; y < grid_2d_grid0.rows; y++) {
        if ((int)grid_2d_grid0.at<uint8_t>(y, x) < num_features_grid_req &&
            (int)mask0_grid.at<uint8_t>(y, x) != 255) {
          valid_locs.emplace_back(x, y);
        }
      }
    ...
    for (auto &kpt : pts0_ext) {
      ...
      int x_grid = (int)(kpt.pt.x / (float)min_px_dist);
      int y_grid = (int)(kpt.pt.y / (float)min_px_dist);
      if (x_grid < 0 || x_grid >= size_close0.width || y_grid < 0 ||
          y_grid >= size_close0.height)
        continue;
      // See if there is a point at this location
      if (grid_2d_close0.at<uint8_t>(y_grid, x_grid) > 127) continue;
      // Else lets add it!
    ...
    // If we have points, do KLT tracking to get the valid projections into the
    // right image
    if (!pts0_new.empty()) {
      // Do our KLT tracking from the left to the right frame of reference
      // NOTE: we have a pretty big window size here since our projection might
      // be bad NOTE: but this might cause failure in cases of repeated textures
      // (eg. checkerboard)
      ...
      for (size_t i = 0; i < pts0_new.size(); i++) {
        ...
        // this location
        //  1) If this is not already in the right image, then we should treat
        //  it as a stereo 2) Otherwise we will treat this as just a monocular
        //  track of the feature
        // TODO: we should check to see if we can combine this new feature and
        // the one in the right
        // TODO: seems if reject features which overlay with right features
        // already we have very poor tracking perf
  ...
  cv::Mat mask1_use = mask1.empty()
                          ? cv::Mat::zeros(img1pyr.at(0).size(), CV_8UC1)
                          : mask1.clone();
  ...
  while (it0 != pts1.end()) {
    ...
    int x_close = (int)(kpt.pt.x / (float)min_px_dist);
    int y_close = (int)(kpt.pt.y / (float)min_px_dist);
    if (x_close < 0 || x_close >= size_close1.width || y_close < 0 ||
        y_close >= size_close1.height) {
      it0 = pts1.erase(it0);
      it1 = ids1.erase(it1);
      continue;
    }
    ...
    if (x_grid < 0 || x_grid >= size_grid1.width || y_grid < 0 ||
        y_grid >= size_grid1.height) {
      it0 = pts1.erase(it0);
      it1 = ids1.erase(it1);
      continue;
    }
    ...
    // NOTE: this means we might have a mono and stereo feature near each other,
    // but that is ok
  ...
  // RIGHT: note that we don't track them to the left as we already did
  // left->right tracking above
  int num_featsneeded_1 = num_features - (int)pts1.size();
  if (num_featsneeded_1 >
      std::min(20, (int)(min_feat_percent * num_features))) {
    // This is old extraction code that would extract from the whole image
    // This can be slow as this will recompute extractions for grid areas that
    // we have max features already std::vector<cv::KeyPoint> pts1_ext;
    // Grider_FAST::perform_griding(img1pyr.at(0), mask1_updated, pts1_ext,
    // num_features, grid_x, grid_y, threshold, true);
    ...
    cv::Mat mask1_grid;
    if (!mask1_use.empty()) {
      cv::resize(mask1_use, mask1_grid, size_grid1, 0.0, 0.0,
                 cv::INTER_NEAREST);
    } else {
    ...
    for (int x = 0; x < grid_2d_grid1.cols; x++) {
      for (int y = 0; y < grid_2d_grid1.rows; y++) {
        if ((int)grid_2d_grid1.at<uint8_t>(y, x) < num_features_grid_req &&
    ...
    for (auto &kpt : pts1_ext) {
      ...
      int x_grid = (int)(kpt.pt.x / (float)min_px_dist);
      int y_grid = (int)(kpt.pt.y / (float)min_px_dist);
      if (x_grid < 0 || x_grid >= size_close1.width || y_grid < 0 ||
          y_grid >= size_close1.height)
        continue;
      // See if there is a point at this location
      if (grid_2d_close1.at<uint8_t>(y_grid, x_grid) > 127) continue;
      // Else lets add it!
      pts1.push_back(kpt);
      size_t temp = ++currid;
      ids1.push_back(temp);
...
void TrackKLT::perform_matching(const std::vector<cv::Mat> &img0pyr,
  ...
  if (pts0.size() < 10) {
    for (size_t i = 0; i < pts0.size(); i++) mask_out.push_back((uchar)0);
    return;
  }
  ...
  const double ransac_thr = 2.0 / max_focallength;
  ...
  const auto f_hull0 = compute_hull_area_and_shrunk(pts0, mask_rsc, kShrinkPx);
  const auto h_hull0 = compute_hull_area_and_shrunk(pts0, mask_hmg, kShrinkPx);
  const auto f_hull1 = compute_hull_area_and_shrunk(pts1, mask_rsc, kShrinkPx);
  const auto h_hull1 = compute_hull_area_and_shrunk(pts1, mask_hmg, kShrinkPx);
  ...
  for (size_t i = 0; i < mask_klt.size(); i++) {
    auto mask = (uchar)((i < mask_klt.size() && mask_klt[i] &&
                         i < mask_rsc.size() && mask_rsc[i])
                            ? 1
                            : 0);
