...
bool FeatureInitializer::single_triangulation(
  ...
  feat->anchor_clone_timestamp =
      feat->timestamps.at(feat->anchor_cam_id).back();
  ...
  ClonePose anchorclone =
      clonesCAM.at(feat->anchor_cam_id).at(feat->anchor_clone_timestamp);
  ...
  for (auto const &pair : feat->timestamps) {
    ...
    for (size_t m = 0; m < feat->timestamps.at(pair.first).size(); m++) {
      ...
      const Eigen::Matrix<double, 3, 3> &R_GtoCi =
          clonesCAM.at(pair.first)
              .at(feat->timestamps.at(pair.first).at(m))
              .Rot();
      const Eigen::Matrix<double, 3, 1> &p_CiinG =
          clonesCAM.at(pair.first)
              .at(feat->timestamps.at(pair.first).at(m))
              .pos();
      ...
      b_i << feat->uvs_norm.at(pair.first).at(m)(0),
          feat->uvs_norm.at(pair.first).at(m)(1), 1;
...
bool FeatureInitializer::single_triangulation_1d(
  ...
  feat->anchor_clone_timestamp =
      feat->timestamps.at(feat->anchor_cam_id).back();
  size_t idx_anchor_bearing =
      feat->timestamps.at(feat->anchor_cam_id).size() - 1;
  ...
  bearing_inA
      << feat->uvs_norm.at(feat->anchor_cam_id).at(idx_anchor_bearing)(0),
  ...
  // Then set the flag for bad (i.e. set z-axis to nan)
  if (p_f(2, 0) < _options.min_dist || p_f(2, 0) > _options.max_dist ||
      std::isnan(p_f.norm())) {
    return false;
  }
...
bool FeatureInitializer::single_gaussnewton(
  ...
  // Get the position of the anchor pose
  const Eigen::Matrix<double, 3, 3> &R_GtoA =
      clonesCAM.at(feat->anchor_cam_id).at(feat->anchor_clone_timestamp).Rot();
  const Eigen::Matrix<double, 3, 1> &p_AinG =
      clonesCAM.at(feat->anchor_cam_id).at(feat->anchor_clone_timestamp).pos();

  // Loop till we have either
  // 1. Reached our max iteration count
  // 2. System is unstable
  // 3. System has converged
  while (runs < _options.max_runs && lam < _options.max_lamda &&
         eps > _options.min_dx) {
    ...
    if (recompute) {
      ...
      for (auto const &pair : feat->timestamps) {
        ...
        for (size_t m = 0; m < feat->timestamps.at(pair.first).size(); m++) {
          ...
          const Eigen::Matrix<double, 3, 3> &R_GtoCi =
              clonesCAM.at(pair.first)
                  .at(feat->timestamps[pair.first].at(m))
                  .Rot();
          const Eigen::Matrix<double, 3, 1> &p_CiinG =
              clonesCAM.at(pair.first)
                  .at(feat->timestamps[pair.first].at(m))
          ...
          double hi1 = R_AtoCi(0, 0) * alpha + R_AtoCi(0, 1) * beta +
                       R_AtoCi(0, 2) + rho * p_AinCi(0, 0);
          double hi2 = R_AtoCi(1, 0) * alpha + R_AtoCi(1, 1) * beta +
                       R_AtoCi(1, 2) + rho * p_AinCi(1, 0);
          double hi3 = R_AtoCi(2, 0) * alpha + R_AtoCi(2, 1) * beta +
                       R_AtoCi(2, 2) + rho * p_AinCi(2, 0);
          // Calculate jacobian
          double d_z1_d_alpha =
              (R_AtoCi(0, 0) * hi3 - hi1 * R_AtoCi(2, 0)) / (pow(hi3, 2));
          double d_z1_d_beta =
              (R_AtoCi(0, 1) * hi3 - hi1 * R_AtoCi(2, 1)) / (pow(hi3, 2));
          double d_z1_d_rho =
              (p_AinCi(0, 0) * hi3 - hi1 * p_AinCi(2, 0)) / (pow(hi3, 2));
          double d_z2_d_alpha =
              (R_AtoCi(1, 0) * hi3 - hi2 * R_AtoCi(2, 0)) / (pow(hi3, 2));
          double d_z2_d_beta =
              (R_AtoCi(1, 1) * hi3 - hi2 * R_AtoCi(2, 1)) / (pow(hi3, 2));
          double d_z2_d_rho =
              (p_AinCi(1, 0) * hi3 - hi2 * p_AinCi(2, 0)) / (pow(hi3, 2));
          Eigen::Matrix<double, 2, 3> H;
          H << d_z1_d_alpha, d_z1_d_beta, d_z1_d_rho, d_z2_d_alpha, d_z2_d_beta,
              d_z2_d_rho;
          // Calculate residual
          Eigen::Matrix<float, 2, 1> z;
          z << hi1 / hi3, hi2 / hi3;
          Eigen::Matrix<float, 2, 1> res =
              feat->uvs_norm.at(pair.first).at(m) - z;
    ...
    // (Hess+lam*Eigen::MatrixXd::Identity(Hess.rows(),
    // Hess.rows())).colPivHouseholderQr().solve(grad);

    // Check if error has gone down
    double cost = compute_error(clonesCAM, feat, alpha + dx(0, 0),
                                beta + dx(1, 0), rho + dx(2, 0));

    // Debug print
    // std::stringstream ss;
    // ss << "run = " << runs << " | cost = " << dx.norm() << " | lamda = " <<
    // lam << " | depth = " << 1/rho << endl; PRINT_DEBUG(ss.str().c_str());
  ...
  // std::stringstream ss;
  // ss << feat->featid << " - max base " << (feat->p_FinA.norm() /
  // base_line_max) << " - z " << feat->p_FinA(2) << std::endl;
