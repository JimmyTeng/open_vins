...
void state_callback(const vio_state_t* state, void* /*user_data*/) {
    ...
    if (state->debug && state->debug->points_slam && state->debug->num_slam > 0) {
        const int n = std::min(state->debug->num_slam, static_cast<int32_t>(OT_VIO_SLAM_POINTS_MAX));
...
int SS_VIO_RegisterSlamCallback(OT_VIO_SlamCallback_t callback, void* user_data) {
    std::lock_guard<std::mutex> lk(g_slam_mtx);
    g_slam_cb = callback;
...
int SS_VIO_GetBuildInfo(SS_VIO_BuildInfo* out) {
  if (!out) return -1;
  build_info_copy_str(out->build_time, sizeof(out->build_time), BUILD_TIMESTAMP);
  build_info_copy_str(out->build_timezone, sizeof(out->build_timezone), BUILD_TIMEZONE);
  build_info_copy_str(out->git_commit, sizeof(out->git_commit), BUILD_GIT_COMMIT);
  build_info_copy_str(out->git_hash, sizeof(out->git_hash), BUILD_GIT_HASH);
  build_info_copy_str(out->git_branch, sizeof(out->git_branch), BUILD_GIT_BRANCH);
  build_info_copy_str(out->git_tag, sizeof(out->git_tag), BUILD_GIT_TAG);
  build_info_copy_str(out->git_status, sizeof(out->git_status), BUILD_GIT_DIRTY);
...
int SS_VIO_FormatBuildInfo(char* buffer, size_t size) {
  ...
  int n = snprintf(tmp, sizeof(tmp),
      ...
      "  Git User:     %s <%s>\n"
      "=================================\n",
      BUILD_TIMESTAMP, BUILD_TIMEZONE,
      BUILD_GIT_COMMIT, BUILD_GIT_HASH,
      BUILD_GIT_BRANCH, BUILD_GIT_TAG, BUILD_GIT_DIRTY,
      BUILD_GIT_USER, BUILD_GIT_EMAIL);
  if (n < 0) return -1;
