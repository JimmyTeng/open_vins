...
bool Propagator::fast_state_propagate(
  ...
  if (prop_data.size() < 2) return false;
  ...
  for (size_t i = 0; i < prop_data.size() - 1; i++) {
    ...
    // 使用当前偏置校正IMU陀螺仪测量值
    Eigen::Vector3d w_hat1 =
        R_GYROtoIMU * Dw * (data_minus.wm - bias_g - Tg * a_hat1);
    Eigen::Vector3d w_hat2 =
        R_GYROtoIMU * Dw * (data_plus.wm - bias_g - Tg * a_hat2);
    ...
    // TODO: should probably track the correlations with the IMU intrinsics if
    // we are calibrating
    // TODO: currently this just does a quick discrete prediction using only the
    // previous marg IMU uncertainty
    ...
    cache_state_est.block(0, 0, 4, 1) =
        rot_2_quat(exp_so3(-w_hat * dt) * R_Gtoi);
    cache_state_est.block(4, 0, 3, 1) =
        p_iinG + v_iinG * dt + 0.5 * R_Gtoi.transpose() * a_hat * dt * dt -
        0.5 * _gravity * dt * dt;
    cache_state_est.block(7, 0, 3, 1) =
  ...
  Eigen::Vector3d last_a =
      R_ACCtoIMU * Da * (prop_data.at(prop_data.size() - 1).am - bias_a);
  Eigen::Vector3d last_w =
      R_GYROtoIMU * Dw *
      (prop_data.at(prop_data.size() - 1).wm - bias_g - Tg * last_a);
  ...
  Eigen::MatrixXd covariance_tmp =
      Phi * cache_state_covariance * Phi.transpose();
...
std::vector<ov_core::ImuData> Propagator::select_imu_readings(
  ...
  for (size_t i = 0; i < imu_data.size() - 1; i++) {
    ...
    // 积分周期开始
    // 如果下一个时间戳大于当前状态时间，且当前时间戳还不大于它
    // 那么应该"分割"当前IMU测量值
    if (imu_data.at(i + 1).timestamp > time0 &&
        imu_data.at(i).timestamp < time0) {
      ov_core::ImuData data = Propagator::interpolate_data(
          imu_data.at(i), imu_data.at(i + 1), time0);
    ...
    // vector 积分周期中间 如果IMU测量值正好在传播周期中间
    // 那么应该将整个测量时间添加到传播向量中
    ...
    // this is the last IMU measurement we need! 积分周期结束
    // 如果当前时间戳大于更新时间
    // 应该将下一个IMU测量值"分割"到更新时间
    // 注意：添加当前时间，然后添加区间结束时间（以便获得dt）
    // 注意：同时跳出循环，因为这是需要的最后一个IMU测量值！
  ...
  if (prop_data.empty()) {
    if (warn)
      PRINT_WARNING(
          YELLOW
          "Propagator::select_imu_readings(): No IMU measurements to propagate "
  ...
  // want to reach) Then we should just "stretch" the last measurement to be the
  // whole period
  // TODO: this really isn't that good of logic, we should fix this so the above
  // logic is exact! 如果没有达到整个积分周期
  // （即，我们拥有的最后一个惯性测量值小于我们想要达到的时间）
  // 那么应该将最后一个测量值"拉伸"到整个周期
  // TODO: 这确实不是很好的逻辑，应该修复它以使上述逻辑精确！
  if (prop_data.at(prop_data.size() - 1).timestamp != time1) {
    if (warn)
      PRINT_DEBUG(YELLOW
                  "Propagator::select_imu_readings(): Missing inertial "
                  "measurements to propagate with (%f sec missing)!\n" RESET,
                  (time1 - imu_data.at(imu_data.size() - 1).timestamp));
    ov_core::ImuData data =
        interpolate_data(imu_data.at(imu_data.size() - 2),
  ...
  // and doing unit tests on it... 遍历并确保没有任何零dt值
  // 这会导致噪声协方差为无穷大
  // TODO: 应该通过正确实现此函数并对其进行单元测试来修复此问题...
  for (size_t i = 0; i < prop_data.size() - 1; i++) {
    if (std::abs(prop_data.at(i + 1).timestamp - prop_data.at(i).timestamp) <
        1e-12) {
      if (warn)
        PRINT_WARNING(YELLOW
                      "Propagator::select_imu_readings(): Zero DT between IMU "
...
void Propagator::predict_and_compute(std::shared_ptr<State> state,
  ...
  // Corrected imu gyro measurements with our current biases and gravity
  // sensitivity 使用当前偏置和重力敏感度校正IMU陀螺仪测量值
  ...
  Eigen::Matrix<double, 3, 18> Xi_sum =
      Eigen::Matrix<double, 3, 18>::Zero(3, 18);
  if (state->_options.integration_method ==
          StateOptions::IntegrationMethod::RK4 ||
      state->_options.integration_method ==
          StateOptions::IntegrationMethod::ANALYTICAL) {
  ...
  if (state->_options.integration_method ==
      StateOptions::IntegrationMethod::ANALYTICAL) {
    predict_mean_analytic(state, dt, w_hat_avg, a_hat_avg, new_q, new_v, new_p,
                          Xi_sum);
  } else if (state->_options.integration_method ==
             StateOptions::IntegrationMethod::RK4) {
    predict_mean_rk4(state, dt, w_hat1, a_hat1, w_hat2, a_hat2, new_q, new_v,
                     new_p);
  } else {
  ...
  // 分配状态转移和连续时间噪声雅可比矩阵
  F = Eigen::MatrixXd::Zero(state->imu_intrinsic_size() + 15,
                            state->imu_intrinsic_size() + 15);
  Eigen::MatrixXd G =
      Eigen::MatrixXd::Zero(state->imu_intrinsic_size() + 15, 12);
  if (state->_options.integration_method ==
          StateOptions::IntegrationMethod::RK4 ||
      state->_options.integration_method ==
          StateOptions::IntegrationMethod::ANALYTICAL) {
    compute_F_and_G_analytic(state, dt, w_hat_avg, a_hat_avg, w_uncorrected,
                             a_uncorrected, new_q, new_v, new_p, Xi_sum, F, G);
  } else {
    compute_F_and_G_discrete(state, dt, w_hat_avg, a_hat_avg, w_uncorrected,
                             a_uncorrected, new_q, new_v, new_p, F, G);
  }
  ...
  // 构建离散噪声协方差矩阵
  // 注意需要将连续时间噪声转换为离散时间
  // Trawny技术报告中的方程(129)和(130)
  Eigen::Matrix<double, 12, 12> Qc = Eigen::Matrix<double, 12, 12>::Zero();
  Qc.block(0, 0, 3, 3) =
      std::pow(_noises.sigma_w, 2) / dt * Eigen::Matrix3d::Identity();
  Qc.block(3, 3, 3, 3) =
      std::pow(_noises.sigma_a, 2) / dt * Eigen::Matrix3d::Identity();
  Qc.block(6, 6, 3, 3) =
      std::pow(_noises.sigma_wb, 2) / dt * Eigen::Matrix3d::Identity();
  Qc.block(9, 9, 3, 3) =
      std::pow(_noises.sigma_ab, 2) / dt * Eigen::Matrix3d::Identity();

  // Compute the noise injected into the state over the interval
  // 计算区间内注入状态的噪声
  Qd = Eigen::MatrixXd::Zero(state->imu_intrinsic_size() + 15,
                             state->imu_intrinsic_size() + 15);
  Qd = G * Qc * G.transpose();
  Qd = 0.5 * (Qd + Qd.transpose());
...
void Propagator::predict_mean_rk4(
    std::shared_ptr<State> state, double dt, const Eigen::Vector3d &w_hat1,
  ...
  Eigen::Vector3d w_alpha = (w_hat2 - w_hat1) / dt;  // 角速度变化率
  Eigen::Vector3d a_jerk = (a_hat2 - a_hat1) / dt;   // 加速度变化率（加加速度）
  ...
  // y+dt ================
  // 最终结果：使用加权平均组合k1, k2, k3, k4
  Eigen::Vector4d dq = quatnorm(dq_0 + (1.0 / 6.0) * k1_q + (1.0 / 3.0) * k2_q +
...
void Propagator::compute_Xi_sum(std::shared_ptr<State> state, double dt,
                                const Eigen::Vector3d &w_hat,
                                const Eigen::Vector3d &a_hat,
                                Eigen::Matrix<double, 3, 18> &Xi_sum) {
  ...
  // 将角速度分解为方向和大小
  double w_norm = w_hat.norm();
  double d_th = w_norm * dt;  // 角度变化量
  Eigen::Vector3d k_hat = Eigen::Vector3d::Zero();
  if (w_norm > 1e-12) {
    k_hat = w_hat / w_norm;  // 角速度方向单位向量
  }
  ...
  Eigen::Matrix3d sK = ov_core::skew_x(k_hat);  // 反对称矩阵
  Eigen::Matrix3d sK2 = sK * sK;
  Eigen::Matrix3d sA = ov_core::skew_x(a_hat);  // 加速度的反对称矩阵
  ...
  Jr_ktok1 = ov_core::Jr_so3(-w_hat * dt);  // 右雅可比矩阵

  // Now begin the integration of each component
  // Based on the delta theta, let's decide which integration will be used
  // 现在开始每个分量的积分
  // 根据角度变化量，决定使用哪种积分方法
  bool small_w = (w_norm < 1.0 / 180 * M_PI / 2);  // 判断是否为小角度
  if (!small_w) {
    // 大角度情况：使用完整解析公式

    // first order rotation integration with constant omega
    // 恒定角速度的一阶旋转积分
    Xi_1 = I_3x3 * dt + (1.0 - cos_dth) / w_norm * sK +
           (dt - sin_dth / w_norm) * sK2;

    // second order rotation integration with constant omega
    // 恒定角速度的二阶旋转积分
    Xi_2 = 1.0 / 2 * d_t2 * I_3x3 + (d_th - sin_dth) / w_norm2 * sK +
           (1.0 / 2 * d_t2 - (1.0 - cos_dth) / w_norm2) * sK2;

    // first order integration with constant omega and constant acc
    // 恒定角速度和恒定加速度的一阶积分
    Xi_3 = 1.0 / 2 * d_t2 * sA + (sin_dth - d_th) / w_norm2 * sA * sK +
           (sin_dth - d_th * cos_dth) / w_norm2 * sK * sA +
           (1.0 / 2 * d_t2 - (1.0 - cos_dth) / w_norm2) * sA * sK2 +
           (1.0 / 2 * d_t2 + (1.0 - cos_dth - d_th * sin_dth) / w_norm2) *
               (sK2 * sA + k_hat.dot(a_hat) * sK) -
           (3 * sin_dth - 2 * d_th - d_th * cos_dth) / w_norm2 *
               k_hat.dot(a_hat) * sK2;

    // second order integration with constant omega and constant acc
    // 恒定角速度和恒定加速度的二阶积分
    Xi_4 = 1.0 / 6 * d_t3 * sA +
           (2 * (1.0 - cos_dth) - d_th2) / (2 * w_norm3) * sA * sK +
           ((2 * (1.0 - cos_dth) - d_th * sin_dth) / w_norm3) * sK * sA +
           ((sin_dth - d_th) / w_norm3 + d_t3 / 6) * sA * sK2 +
           ((d_th - 2 * sin_dth + 1.0 / 6 * d_th3 + d_th * cos_dth) / w_norm3) *
               (sK2 * sA + k_hat.dot(a_hat) * sK) +
           (4 * cos_dth - 4 + d_th2 + d_th * sin_dth) / w_norm3 *
               k_hat.dot(a_hat) * sK2;
    ...
    // first order integration with constant omega and constant acc
    // 恒定角速度和恒定加速度的一阶积分（小角度近似）
    Xi_3 = 1.0 / 2 * d_t2 *
           (sA + sin_dth * (-sA * sK + sK * sA + k_hat.dot(a_hat) * sK2) +
            (1.0 - cos_dth) * (sA * sK2 + sK2 * sA + k_hat.dot(a_hat) * sK));

    // second order integration with constant omega and constant acc
    // 恒定角速度和恒定加速度的二阶积分（小角度近似）
    Xi_4 = 1.0 / 3 * dt * Xi_3;
  }
  ...
  // 存储积分参数
  Xi_sum.setZero();
  Xi_sum.block(0, 0, 3, 3) = R_ktok1;   // 旋转矩阵
  Xi_sum.block(0, 3, 3, 3) = Xi_1;      // 一阶旋转积分
  Xi_sum.block(0, 6, 3, 3) = Xi_2;      // 二阶旋转积分
  Xi_sum.block(0, 9, 3, 3) = Jr_ktok1;  // 右雅可比矩阵
  Xi_sum.block(0, 12, 3, 3) = Xi_3;     // 一阶加速度积分
  Xi_sum.block(0, 15, 3, 3) = Xi_4;     // 二阶加速度积分
}

void Propagator::predict_mean_analytic(std::shared_ptr<State> state, double dt,
                                       const Eigen::Vector3d &w_hat,
                                       const Eigen::Vector3d &a_hat,
                                       Eigen::Vector4d &new_q,
                                       Eigen::Vector3d &new_v,
  ...
  new_v =
      state->_imu->vel() + R_Gtok.transpose() * Xi_1 * a_hat - _gravity * dt;
  new_p = state->_imu->pos() + state->_imu->vel() * dt +
          R_Gtok.transpose() * Xi_2 * a_hat - 0.5 * _gravity * dt * dt;
}

void Propagator::compute_F_and_G_analytic(
    std::shared_ptr<State> state, double dt, const Eigen::Vector3d &w_hat,
    const Eigen::Vector3d &a_hat, const Eigen::Vector3d &w_uncorrected,
    const Eigen::Vector3d &a_uncorrected, const Eigen::Vector4d &new_q,
    const Eigen::Vector3d &new_v, const Eigen::Vector3d &new_p,
  ...
  // If we are doing calibration, we can define their "local" id in the state
  // transition
  int Dw_id = -1;
  int Da_id = -1;
  int Tg_id = -1;
  ...
  // The change in the orientation from the end of the last prop to the current
  // prop This is needed since we need to include the "k-th" updated orientation
  // information
  ...
  Eigen::Vector3d w_k =
      R_wtoI * Dw * w_uncorrected;  // contains gravity correction already
  ...
  F.block(p_id, th_id, 3, 3) =
      -skew_x(new_p - p_k - v_k * dt + 0.5 * _gravity * dt * dt) *
      R_k.transpose();
  F.block(v_id, th_id, 3, 3) =
  ...
  // for ba
  F.block(th_id, ba_id, 3, 3) =
      dR_ktok1 * Jr_ktok1 * dt * R_wtoI * Dw * Tg * R_atoI * Da;
  F.block(p_id, ba_id, 3, 3) =
      -R_k.transpose() * (Xi_2 + Xi_4 * R_wtoI * Dw * Tg) * R_atoI * Da;
  F.block(v_id, ba_id, 3, 3) =
      -R_k.transpose() * (Xi_1 + Xi_3 * R_wtoI * Dw * Tg) * R_atoI * Da;
  ...
  if (Dw_id != -1) {
    ...
    F.block(th_id, Dw_id, 3, state->_calib_imu_dw->size()) =
        dR_ktok1 * Jr_ktok1 * dt * R_wtoI * H_Dw;
    F.block(p_id, Dw_id, 3, state->_calib_imu_dw->size()) =
        -R_k.transpose() * Xi_4 * R_wtoI * H_Dw;
    F.block(v_id, Dw_id, 3, state->_calib_imu_dw->size()) =
        -R_k.transpose() * Xi_3 * R_wtoI * H_Dw;
    F.block(Dw_id, Dw_id, state->_calib_imu_dw->size(),
            state->_calib_imu_dw->size())
        .setIdentity();
  }
  ...
  if (Da_id != -1) {
    ...
    F.block(th_id, Da_id, 3, state->_calib_imu_da->size()) =
        -dR_ktok1 * Jr_ktok1 * dt * R_wtoI * Dw * Tg * R_atoI * H_Da;
    F.block(p_id, Da_id, 3, state->_calib_imu_da->size()) =
        R_k.transpose() * (Xi_2 + Xi_4 * R_wtoI * Dw * Tg) * R_atoI * H_Da;
    F.block(v_id, Da_id, 3, state->_calib_imu_da->size()) =
        R_k.transpose() * (Xi_1 + Xi_3 * R_wtoI * Dw * Tg) * R_atoI * H_Da;
    F.block(Da_id, Da_id, state->_calib_imu_da->size(),
            state->_calib_imu_da->size())
        .setIdentity();
  }
  ...
  if (Tg_id != -1) {
    ...
    F.block(th_id, Tg_id, 3, state->_calib_imu_tg->size()) =
        -dR_ktok1 * Jr_ktok1 * dt * R_wtoI * Dw * H_Tg;
    F.block(p_id, Tg_id, 3, state->_calib_imu_tg->size()) =
        R_k.transpose() * Xi_4 * R_wtoI * Dw * H_Tg;
    F.block(v_id, Tg_id, 3, state->_calib_imu_tg->size()) =
        R_k.transpose() * Xi_3 * R_wtoI * Dw * H_Tg;
    F.block(Tg_id, Tg_id, state->_calib_imu_tg->size(),
            state->_calib_imu_tg->size())
        .setIdentity();
  }

  // begin to add the state transition matrix for the R_ACCtoIMU part
  if (th_atoI_id != -1) {
    F.block(th_id, th_atoI_id, 3, 3) =
        -dR_ktok1 * Jr_ktok1 * dt * R_wtoI * Dw * Tg * ov_core::skew_x(a_k);
    F.block(p_id, th_atoI_id, 3, 3) = R_k.transpose() *
                                      (Xi_2 + Xi_4 * R_wtoI * Dw * Tg) *
                                      ov_core::skew_x(a_k);
    F.block(v_id, th_atoI_id, 3, 3) = R_k.transpose() *
                                      (Xi_1 + Xi_3 * R_wtoI * Dw * Tg) *
                                      ov_core::skew_x(a_k);
  ...
  if (th_wtoI_id != -1) {
    F.block(th_id, th_wtoI_id, 3, 3) =
        dR_ktok1 * Jr_ktok1 * dt * ov_core::skew_x(w_k);
    F.block(p_id, th_wtoI_id, 3, 3) =
        -R_k.transpose() * Xi_4 * ov_core::skew_x(w_k);
    F.block(v_id, th_wtoI_id, 3, 3) =
        -R_k.transpose() * Xi_3 * ov_core::skew_x(w_k);
...
void Propagator::compute_F_and_G_discrete(
    std::shared_ptr<State> state, double dt, const Eigen::Vector3d &w_hat,
    const Eigen::Vector3d &a_hat, const Eigen::Vector3d &w_uncorrected,
    const Eigen::Vector3d &a_uncorrected, const Eigen::Vector4d &new_q,
    const Eigen::Vector3d &new_v, const Eigen::Vector3d &new_p,
  ...
  // This is the change in the orientation from the end of the last prop to the
  // current prop This is needed since we need to include the "k-th" updated
  ...
  // for theta
  F.block(th_id, th_id, 3, 3) = dR_ktok1;
  // F.block(th_id, bg_id, 3, 3) = -dR_ktok1 * Jr_so3(w_hat * dt) * dt *
  // R_wtoI_fej * Dw_fej;
  F.block(th_id, bg_id, 3, 3) = -dR_ktok1 * Jr_ktok1 * dt * R_wtoI * Dw;
  F.block(th_id, ba_id, 3, 3) =
      dR_ktok1 * Jr_ktok1 * dt * R_wtoI * Dw * Tg * R_atoI * Da;
  ...
  if (Da_id != -1) {
    Eigen::MatrixXd H_Da = compute_H_Da(state, a_uncorrected);
    F.block(th_id, Da_id, 3, state->_calib_imu_da->size()) =
        -dR_ktok1 * Jr_ktok1 * dt * R_wtoI * Tg * R_atoI * H_Da;
    F.block(p_id, Da_id, 3, state->_calib_imu_da->size()) =
        0.5 * R_k.transpose() * dt * dt * R_atoI * H_Da;
    F.block(v_id, Da_id, 3, state->_calib_imu_da->size()) =
        R_k.transpose() * dt * R_atoI * H_Da;
    F.block(Da_id, Da_id, state->_calib_imu_da->size(),
            state->_calib_imu_da->size())
        .setIdentity();
  }

  // begin to add the state transition matrix for the gravity sensitivity Tg
  // part
  if (Tg_id != -1) {
    Eigen::MatrixXd H_Tg = compute_H_Tg(state, a_k);
    F.block(th_id, Tg_id, 3, state->_calib_imu_tg->size()) =
  ...
  if (th_atoI_id != -1) {
    F.block(th_id, th_atoI_id, 3, 3) =
        -dR_ktok1 * Jr_ktok1 * dt * R_wtoI * Dw * Tg * ov_core::skew_x(a_k);
    F.block(p_id, th_atoI_id, 3, 3) =
        0.5 * R_k.transpose() * dt * dt * ov_core::skew_x(a_k);
    F.block(v_id, th_atoI_id, 3, 3) =
        R_k.transpose() * dt * ov_core::skew_x(a_k);
    F.block(th_atoI_id, th_atoI_id, 3, 3).setIdentity();
  }
