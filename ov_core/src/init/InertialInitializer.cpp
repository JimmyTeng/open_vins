...
bool InertialInitializer::initialize(
  ...
  if (params.init_max_disparity > 0) {
    // 获取从当前图像到前一图像的视差统计信息
    // 仅计算初始化周期最旧一半的视差
    double newest_time_allowed =
        newest_cam_time - 0.5 * params.init_window_time;
    ...
    FeatureHelper::compute_disparity(_db, avg_disp0, var_disp0, num_features0,
                                     newest_time_allowed);
    // 计算后半段窗口的视差（从中间时间到最新时间）
    FeatureHelper::compute_disparity(_db, avg_disp1, var_disp1, num_features1,
                                     newest_cam_time, newest_time_allowed);
  ...
  bool has_jerk = (!disparity_detected_moving_1to0 &&
                   disparity_detected_moving_2to1);  // 检测到抖动：从静止到运动
  bool is_still =
      (!disparity_detected_moving_1to0 &&
       !disparity_detected_moving_2to1);  // 完全静止：两个时间段都静止
  if (print_debug) {
    PRINT_DEBUG(YELLOW
                "[InertialInitializer]: 检测到抖动: %d, 完全静止: %d\n" RESET,
                has_jerk, is_still);
  }
  // 使用静态初始化器的条件：
  // 1. 检测到抖动且等待抖动，或
  // 2. 完全静止且不等待抖动
  // 并且IMU阈值大于0
  if (((has_jerk && wait_for_jerk) || (is_still && !wait_for_jerk)) &&
      params.init_imu_thresh > 0.0) {
    return init_static->initialize(timestamp, covariance, order, t_imu,
                                   wait_for_jerk);
    ...
    if (init_dynamic) {
      ...
      return init_dynamic->initialize(timestamp, covariance, order, t_imu,
