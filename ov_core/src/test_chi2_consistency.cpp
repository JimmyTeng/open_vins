 ...
 *
 * 编译方式（需安装 Boost）:
 *   g++ -std=c++17 -O2 -I. test_chi2_consistency.cpp -o test_chi2_consistency
 * -lboost_math_c99
 *
...
int main() {
  const int test_dfs[] = {1,  2,   3,   5,   10,  15,  20,   25,
                          50, 100, 200, 300, 500, 999, 1000, 2000};
  const int n_tests = sizeof(test_dfs) / sizeof(test_dfs[0]);

  std::cout << "=== 卡方 0.95 分位数一致性检验 ===\n";
  std::cout << std::fixed << std::setprecision(6);
  std::cout << std::setw(6) << "df" << std::setw(16) << "手写实现"
            << std::setw(16) << "Boost.Math" << std::setw(14) << "绝对误差"
            << std::setw(12) << "相对误差%\n";
  std::cout << "------------------------------------------------------------\n";
  ...
  for (int i = 0; i < n_tests; i++) {
    ...
    std::cout << std::setw(6) << df << std::setw(16) << ours << std::setw(16)
              << boost_val << std::setw(14) << abs_err << std::setw(11)
              << rel_err << "%" << (ok ? "  OK" : "  FAIL") << "\n";
#else
    std::cout
        << std::setw(6) << df << std::setw(16) << ours
