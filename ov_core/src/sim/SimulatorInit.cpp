...
SimulatorInit::SimulatorInit(InertialInitializerOptions &params_) {
  ...
  // NOTE: We need to explicitly create a copy of our shared pointers to the
  // camera objects NOTE: Otherwise if we perturb it would also change our
  // "true" parameters
  this->params = params_;
  ...
  for (auto const &tmp : params_.camera_intrinsics) {
    ...
    if (tmp_cast != nullptr) {
      params.camera_intrinsics.insert(
          {tmp.first, std::make_shared<ov_core::CamEqui>(tmp.second->w(),
                                                         tmp.second->h())});
      params.camera_intrinsics.at(tmp.first)->set_value(
          params_.camera_intrinsics.at(tmp.first)->get_value());
    } else {
      params.camera_intrinsics.insert(
          {tmp.first, std::make_shared<ov_core::CamRadtan>(tmp.second->w(),
                                                           tmp.second->h())});
      params.camera_intrinsics.at(tmp.first)->set_value(
          params_.camera_intrinsics.at(tmp.first)->get_value());
    }
  ...
  if (!success_pose_init) {
    PRINT_ERROR(
        RED "[SIM]: unable to find the first pose in the spline...\n" RESET);
  ...
  while (true) {
    ...
    if (!success_pose) {
      PRINT_ERROR(RED
                  "[SIM]: unable to find jolt in the groundtruth data to "
                  "initialize at\n" RESET);
      std::exit(EXIT_FAILURE);
    }
  ...
  // We will create synthetic camera frames and ensure that each has enough
  // features double dt = 0.25/freq_cam;
  double dt = 0.25;
  ...
  for (int i = 0; i < params.num_cameras; i++) {
    ...
    while (true) {
      ...
      std::vector<std::pair<size_t, Eigen::VectorXf>> uvs =
          project_pointcloud(R_GtoI, p_IinG, i, featmap);
      // If we do not have enough, generate more
      if ((int)uvs.size() < params.init_max_features) {
        generate_points(R_GtoI, p_IinG, i, featmap,
                        params.init_max_features - (int)uvs.size());
      }
    ...
    // Debug print
    PRINT_DEBUG(
        "[SIM]: Generated %d map features in total over %d frames (camera "
        "%d)\n",
...
void SimulatorInit::perturb_parameters(InertialInitializerOptions &params_) {
  ...
  // Camera IMU offset
  // params_.calib_camimu_dt = 0.005 * w(gen_state_perturb) +
  ...
  for (int i = 0; i < params_.num_cameras; i++) {
    ...
    // params_.camera_intrinsics.at(i)->get_value(); for (int r = 0; r < 4; r++)
    // {
    ...
    // w_vec << 0.001 * w(gen_state_perturb), 0.001 * w(gen_state_perturb),
    // 0.001 * w(gen_state_perturb); params_.camera_extrinsics.at(i).block(0, 0,
    // 4, 1) =
    //    rot_2_quat(exp_so3(w_vec) *
    //    quat_2_Rot(params.camera_extrinsics.at(i).block(0, 0, 4, 1)));

    // Our camera extrinsics transform (position)
    // for (int r = 4; r < 7; r++) {
    //  params_.camera_extrinsics.at(i)(r) = 0.005 * w(gen_state_perturb) +
    //  params.camera_extrinsics.at(i)(r);
    // }
  }
}
...
bool SimulatorInit::get_next_imu(double &time_imu, Eigen::Vector3d &wm,
                                 Eigen::Vector3d &am) {
  // Return if the camera measurement should go before us
  if (timestamp_last_cam + 1.0 / params.sim_freq_cam <
...
bool SimulatorInit::get_next_cam(
  ...
  for (int i = 0; i < params.num_cameras; i++) {
    ...
    if ((int)uvs.size() < params.init_max_features) {
      PRINT_WARNING(YELLOW
                    "[SIM]: cam %d was unable to generate enough features (%d "
                    "< %d projections)\n" RESET,
                    (int)i, (int)uvs.size(), params.init_max_features);
    }
...
SimulatorInit::project_pointcloud(
  ...
  Eigen::Matrix<double, 3, 1> p_IinC =
      params.camera_extrinsics.at(camid).block(4, 0, 3, 1);
...
void SimulatorInit::generate_points(
    const Eigen::Matrix3d &R_GtoI, const Eigen::Vector3d &p_IinG, int camid,
  ...
  for (int i = 0; i < numpts; i++) {
    ...
    std::uniform_real_distribution<double> gen_depth(
        params.sim_min_feature_gen_distance,
        params.sim_max_feature_gen_distance);
