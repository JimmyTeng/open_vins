...
void Simulator::perturb_parameters(std::mt19937 gen_state,
  ...
  for (int i = 0; i < params_.state_options.num_cameras; i++) {
    ...
    params_.camera_extrinsics.at(i).block(0, 0, 4, 1) = rot_2_quat(
        exp_so3(w_vec) *
        quat_2_Rot(params_.camera_extrinsics.at(i).block(0, 0, 4, 1)));
  ...
  if (params_.state_options.do_calib_imu_intrinsics) {
    ...
    if (params_.state_options.imu_model == StateOptions::ImuModel::KALIBR) {
      ...
      params_.q_GYROtoIMU =
          rot_2_quat(exp_so3(w_vec) * quat_2_Rot(params_.q_GYROtoIMU));
    } else {
      Eigen::Vector3d w_vec;
      w_vec << 0.002 * w(gen_state), 0.002 * w(gen_state), 0.002 * w(gen_state);
      params_.q_ACCtoIMU =
          rot_2_quat(exp_so3(w_vec) * quat_2_Rot(params_.q_ACCtoIMU));
    }
  }
...
bool Simulator::get_next_imu(double &time_imu, Eigen::Vector3d &wm,
                             Eigen::Vector3d &am) {
  ...
  Eigen::Vector3d omega_inGYRO =
      Tw * quat_2_Rot(params.q_GYROtoIMU).transpose() * omega_inI +
      Tg * accel_inI;
  Eigen::Vector3d accel_inACC =
      Ta * quat_2_Rot(params.q_ACCtoIMU).transpose() * accel_inI;
  ...
  if (has_skipped_first_bias) {
    ...
    true_bias_gyro(0) +=
        params.imu_noises.sigma_wb * std::sqrt(dt) * w(gen_meas_imu);
    true_bias_gyro(1) +=
        params.imu_noises.sigma_wb * std::sqrt(dt) * w(gen_meas_imu);
    true_bias_gyro(2) +=
        params.imu_noises.sigma_wb * std::sqrt(dt) * w(gen_meas_imu);
    true_bias_accel(0) +=
        params.imu_noises.sigma_ab * std::sqrt(dt) * w(gen_meas_imu);
    true_bias_accel(1) +=
        params.imu_noises.sigma_ab * std::sqrt(dt) * w(gen_meas_imu);
    true_bias_accel(2) +=
        params.imu_noises.sigma_ab * std::sqrt(dt) * w(gen_meas_imu);
...
bool Simulator::get_next_cam(
  ...
  for (int i = 0; i < params.state_options.num_cameras; i++) {
    ...
    for (size_t j = 0; j < uvs.size(); j++) {
      uvs.at(j).second(0) +=
          params.msckf_options.sigma_pix * w(gen_meas_cams.at(i));
      uvs.at(j).second(1) +=
          params.msckf_options.sigma_pix * w(gen_meas_cams.at(i));
    }
