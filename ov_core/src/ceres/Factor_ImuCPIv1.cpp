...
Factor_ImuCPIv1::Factor_ImuCPIv1(
    double deltatime, Eigen::Vector3d &grav, Eigen::Vector3d &alpha,
    Eigen::Vector3d &beta, Eigen::Vector4d &q_KtoK1, Eigen::Vector3d &ba_lin,
    Eigen::Vector3d &bg_lin, Eigen::Matrix3d &J_q, Eigen::Matrix3d &J_beta,
  ...
  // Check that we have a valid covariance matrix that we can get the
  // information of
  Eigen::MatrixXd I =
...
bool Factor_ImuCPIv1::Evaluate(double const *const *parameters,
  ...
  Eigen::Vector4d q_res_minus =
      ov_core::quat_multiply(q_1_to_2, ov_core::Inv(q_breve));
  ...
  res.block(6, 0, 3, 1) =
      R_1 * (v_2 - v_1 + gravity * dt) - J_b * dbw - H_b * dba - beta;
  res.block(9, 0, 3, 1) = b_a2 - b_a1;
  res.block(12, 0, 3, 1) =
      R_1 * (p_2 - p_1 - v_1 * dt + .5 * gravity * std::pow(dt, 2)) -
      J_a * dbw - H_a * dba - alpha;
  res = sqrtI * res;
  ...
  if (jacobians) {
    ...
    Eigen::Matrix<double, 15, 30> Jacobian =
        Eigen::Matrix<double, 15, 30>::Zero();

    // Quick identity
    Eigen::Matrix<double, 3, 3> eye = Eigen::MatrixXd::Identity(3, 3);
    Eigen::Matrix<double, 4, 1> q_meas_plus =
        ov_core::quat_multiply(ov_core::Inv(q_breve), q_b);

    // Dtheta wrt theta 1
    Jacobian.block(0, 0, 3, 3) = -(
        (q_1_to_2(3, 0) * eye - ov_core::skew_x(q_1_to_2.block(0, 0, 3, 1))) *
            (q_meas_plus(3, 0) * eye +
             ov_core::skew_x(q_meas_plus.block(0, 0, 3, 1))) -
        q_1_to_2.block(0, 0, 3, 1) * q_meas_plus.block(0, 0, 3, 1).transpose());

    // Dtheta wrt theta 2
    Jacobian.block(0, 15, 3, 3) =
        q_res_plus(3, 0) * eye + ov_core::skew_x(q_res_plus.block(0, 0, 3, 1));

    // Dtheta wrt bw 1
    Jacobian.block(0, 3, 3, 3) =
        (q_res_minus(3, 0) * eye -
    ...
    Jacobian.block(6, 0, 3, 3) =
        ov_core::skew_x(R_1 * (v_2 - v_1 + gravity * dt));
    ...
    Jacobian.block(12, 0, 3, 3) = ov_core::skew_x(
        R_1 * (p_2 - p_1 - v_1 * dt + .5 * gravity * std::pow(dt, 2)));
    // Dposition wrt v 1
    ...
    // Th1
    if (jacobians[0]) {
      Eigen::Map<Eigen::Matrix<double, 15, 4, Eigen::RowMajor>> J_th1(
          jacobians[0], 15, 4);
    ...
    // Th2
    if (jacobians[5]) {
      Eigen::Map<Eigen::Matrix<double, 15, 4, Eigen::RowMajor>> J_th2(
          jacobians[5], 15, 4);
    ...
    // bw1
    if (jacobians[1]) {
      Eigen::Map<Eigen::Matrix<double, 15, 3, Eigen::RowMajor>> J_bw1(
          jacobians[1], 15, 3);
      J_bw1.block(0, 0, 15, 3) = Jacobian.block(0, 3, 15, 3);
    }
    // bw2
    if (jacobians[6]) {
      Eigen::Map<Eigen::Matrix<double, 15, 3, Eigen::RowMajor>> J_bw2(
          jacobians[6], 15, 3);
      J_bw2.block(0, 0, 15, 3) = Jacobian.block(0, 18, 15, 3);
    }
    // v1
    if (jacobians[2]) {
      Eigen::Map<Eigen::Matrix<double, 15, 3, Eigen::RowMajor>> J_v1(
          jacobians[2], 15, 3);
      J_v1.block(0, 0, 15, 3) = Jacobian.block(0, 6, 15, 3);
    }
    // v2
    if (jacobians[7]) {
      Eigen::Map<Eigen::Matrix<double, 15, 3, Eigen::RowMajor>> J_v2(
          jacobians[7], 15, 3);
      J_v2.block(0, 0, 15, 3) = Jacobian.block(0, 21, 15, 3);
    }
    // ba1
    if (jacobians[3]) {
      Eigen::Map<Eigen::Matrix<double, 15, 3, Eigen::RowMajor>> J_ba1(
          jacobians[3], 15, 3);
      J_ba1.block(0, 0, 15, 3) = Jacobian.block(0, 9, 15, 3);
    }
    // ba2
    if (jacobians[8]) {
      Eigen::Map<Eigen::Matrix<double, 15, 3, Eigen::RowMajor>> J_ba2(
          jacobians[8], 15, 3);
      J_ba2.block(0, 0, 15, 3) = Jacobian.block(0, 24, 15, 3);
    }
    // p1
    if (jacobians[4]) {
      Eigen::Map<Eigen::Matrix<double, 15, 3, Eigen::RowMajor>> J_p1(
          jacobians[4], 15, 3);
      J_p1.block(0, 0, 15, 3) = Jacobian.block(0, 12, 15, 3);
    }
    // p2
    if (jacobians[9]) {
      Eigen::Map<Eigen::Matrix<double, 15, 3, Eigen::RowMajor>> J_p2(
          jacobians[9], 15, 3);
