...
bool Factor_ImageReprojCalib::Evaluate(double const *const *parameters,
  ...
  Eigen::Matrix<double, 8, 1> camera_vals =
      Eigen::Map<const Eigen::Matrix<double, 8, 1>>(parameters[5]);
  ...
  // NOTE: we need to reformulate into a zero error constraint 0 = f(x) + n -
  // uv_meas NOTE: if it was the other way (0 = uv_meas - f(x) - n) all the
  // Jacobians would need to be flipped
  ...
  if (jacobians) {
    ...
    H_dzn_dpfc << 1.0 / p_FinCi(2), 0, -p_FinCi(0) / std::pow(p_FinCi(2), 2), 0,
        1.0 / p_FinCi(2), -p_FinCi(1) / std::pow(p_FinCi(2), 2);
    Eigen::MatrixXd H_dz_dpfc = H_dz_dzn * H_dzn_dpfc;

    // Jacobian wrt q_GtoIi
    if (jacobians[0]) {
      Eigen::Map<Eigen::Matrix<double, 2, 4, Eigen::RowMajor>> jacobian(
          jacobians[0]);
      jacobian.block(0, 0, 2, 3) =
          H_dz_dpfc * R_ItoC * ov_core::skew_x(p_FinIi);
      jacobian.block(0, 3, 2, 1).setZero();
    }

    // Jacobian wrt p_IiinG
    if (jacobians[1]) {
      Eigen::Map<Eigen::Matrix<double, 2, 3, Eigen::RowMajor>> jacobian(
          jacobians[1]);
    ...
    if (jacobians[3]) {
      Eigen::Map<Eigen::Matrix<double, 2, 4, Eigen::RowMajor>> jacobian(
          jacobians[3]);
      jacobian.block(0, 0, 2, 3) =
          H_dz_dpfc * ov_core::skew_x(R_ItoC * p_FinIi);
      jacobian.block(0, 3, 2, 1).setZero();
    }
    ...
    if (jacobians[5]) {
      Eigen::Map<Eigen::Matrix<double, 2, 8, Eigen::RowMajor>> jacobian(
          jacobians[5]);
