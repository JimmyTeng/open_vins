...
void UpdaterHelper::get_feature_jacobian_representation(
    std::shared_ptr<State> state, UpdaterHelperFeature &feature,
  ...
  if (feature.feat_representation ==
    ...
    Eigen::Matrix<double, 3, 1> p_FinG =
        (state->_options.do_fej) ? feature.p_FinG_fej : feature.p_FinG;
  ...
  Eigen::Matrix3d R_ItoC =
      state->_calib_IMUtoCAM.at(feature.anchor_cam_id)->Rot();
  Eigen::Vector3d p_IinC =
      state->_calib_IMUtoCAM.at(feature.anchor_cam_id)->pos();
  Eigen::Matrix3d R_GtoI =
      state->_clones_IMU.at(feature.anchor_clone_timestamp)->Rot();
  Eigen::Vector3d p_IinG =
      state->_clones_IMU.at(feature.anchor_clone_timestamp)->pos();
...
void UpdaterHelper::get_feature_jacobian_full(
  ...
  for (auto const &pair : feature.timestamps) {
    ...
    std::shared_ptr<PoseJPL> calibration =
        state->_calib_IMUtoCAM.at(pair.first);
    ...
    for (size_t m = 0; m < feature.timestamps[pair.first].size(); m++) {
      ...
      std::shared_ptr<PoseJPL> clone_Ci =
          state->_clones_IMU.at(feature.timestamps[pair.first].at(m));
  ...
  if (LandmarkRepresentation::is_relative_representation(
    ...
    std::shared_ptr<PoseJPL> clone_Ai =
        state->_clones_IMU.at(feature.anchor_clone_timestamp);
    ...
    if (state->_options.do_calib_camera_pose) {
      ...
      std::shared_ptr<PoseJPL> clone_calib =
          state->_calib_IMUtoCAM.at(feature.anchor_cam_id);
  ...
  for (auto const &pair : feature.timestamps) {
    ...
    for (size_t m = 0; m < feature.timestamps[pair.first].size(); m++) {
      ...
      uv_dist =
          state->_cam_intrinsics_cameras.at(pair.first)->distort_d(uv_norm);

      // 残差
      Eigen::Vector2d uv_m;
      uv_m << (double)feature.uvs[pair.first].at(m)(0),
          (double)feature.uvs[pair.first].at(m)(1);
      ...
      state->_cam_intrinsics_cameras.at(pair.first)
      ...
      if (state->_options.do_calib_camera_pose) {
        ...
        // 应用链式法则并将其添加到大型雅可比矩阵
        H_x.block(2 * c, map_hx[calibration], 2, calibration->size())
            .noalias() += dz_dpfc * dpfc_dcalib;
      }
...
void UpdaterHelper::nullspace_project_inplace(Eigen::MatrixXd &H_f,
  ...
  for (int n = 0; n < H_f.cols(); ++n) {
    for (int m = (int)H_f.rows() - 1; m > n; m--) {
      ...
      (H_f.block(m - 1, n, 2, H_f.cols() - n))
          .applyOnTheLeft(0, 1, tempHo_GR.adjoint());
      (H_x.block(m - 1, 0, 2, H_x.cols()))
          .applyOnTheLeft(0, 1, tempHo_GR.adjoint());
