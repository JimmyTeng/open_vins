...
StreamDataLoader::StreamDataLoader(YUVParser* yuv_parser, const std::string& dump_yuv_dir,
                                   const std::string& imu_file, size_t max_frame_cache)
    ...
    std::sort(schedule_.begin(), schedule_.end(),
              [](const ScheduleItem& a, const ScheduleItem& b) { return a.timestamp < b.timestamp; });
}
...
void StreamDataLoader::start() { reader_thread_ = std::thread(&StreamDataLoader::readerLoop, this); }
...
void StreamDataLoader::readerLoop() {
    for (size_t i = 0; i < schedule_.size() && !done_; ++i) {
        ...
        if (!yuv_parser_->readFrameAt(ref.yuv_path, ref.frame_index, frame_data)) continue;
        FrameData fd;
        fd.info = ref.info;
