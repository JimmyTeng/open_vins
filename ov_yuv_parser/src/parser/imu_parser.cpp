...
std::vector<IMUData> IMUParser::parseIMUFile(const std::string& imu_file) {
  ...
  while (std::getline(file, line)) {
    ...
    IMUData imu;
    // Parse format: index:X, curPts:Y, timeStamp:Z, accX:..., accY:...,
    // accZ:...,
    //               gyroX:..., gyroY:..., gyroZ:..., accPts:..., gyroTemp:...,
    //               accTemp:...
...
void IMUParser::printIMUData(const IMUData& imu) {
  std::cout << "IMU[" << imu.index << "] timestamp=" << imu.timeStamp
            << " acc=(" << std::fixed << std::setprecision(3) << imu.accX << ","
            << imu.accY << "," << imu.accZ << ")"
...
void IMUParser::printIMUStats(const std::vector<IMUData>& imu_data) {
  ...
  std::cout << "  Time range: "
            << (imu_data.back().timeStamp - imu_data[0].timeStamp) << " pts"
            << std::endl;
}
