...
YUVParser::YUVParser(int width, int height) : width_(width), height_(height) {
...
std::vector<FrameInfo> YUVParser::parseFrameInfo(const std::string& info_file) {
  ...
  while (std::getline(file, line)) {
    ...
    if (!line.empty() && static_cast<unsigned char>(line[0]) == 0xEF) {
      // UTF-8 BOM: EF BB BF
      if (line.size() >= 3 && static_cast<unsigned char>(line[1]) == 0xBB &&
    ...
    iss >> info.filename >> info.frame_count >> info.vi_pts >> info.exp_time >>
...
bool YUVParser::readFrame(std::ifstream& file,
  ...
  if (file.gcount() != frame_size_) {
    std::cerr << "Error: Failed to read complete frame. Read " << file.gcount()
              << " bytes, expected " << frame_size_ << std::endl;
    return false;
  }
...
bool YUVParser::readFrameAt(const std::string& yuv_file_path, int frame_index,
                            std::vector<unsigned char>& frame_data) {
  ...
  std::streampos seek_pos =
      static_cast<std::streampos>(frame_index) * frame_size_;
...
std::vector<std::vector<unsigned char>> YUVParser::parseYUVFile(
    const std::string& yuv_file, int num_frames) {
...
std::vector<FrameData> YUVParser::parseAllFrames(
...
int YUVParser::exportFramesToPNG(const std::string& dump_yuv_dir,
  ...
  if (stat(dump_yuv_dir.c_str(), &info) != 0 || !(info.st_mode & S_IFDIR)) {
    std::cerr << "Error: Directory does not exist: " << dump_yuv_dir
              << std::endl;
    return -1;
  }

  // Create output directory (parallel to dump_yuv, replace "dump_yuv" with
  // "png")
  ...
  std::cout << "Found " << all_frames.size() << " frames. Exporting to PNG..."
            << std::endl;
...
int YUVParser::playFramesStepByStep(const std::string& dump_yuv_dir,
  ...
  if (!merged_data.empty()) {
    std::cout << "时间范围: " << merged_data[0].timestamp << " 到 "
              << merged_data.back().timestamp << std::endl;
  }
#if OPENVINS_HAVE_OPENCV_HIGHGUI
  std::cout << "\n按任意键继续播放下一帧，按 'q' 键退出\n" << std::endl;
#else
  ...
  for (size_t i = 0; i < merged_data.size(); ++i) {
    ...
    if (data.type == TimestampedData::FRAME) {
      ...
      if (frame_mat.empty()) {
        std::cerr << "Warning: 无法转换第 " << frame_count << " 帧图像"
                  << std::endl;
        continue;
      }
      cv::Mat display_frame = frame_mat.clone();
      std::stringstream ss;
      ss << "Frame #" << frame_count << " | Timestamp: " << data.timestamp
      ...
      std::cout << "[" << i + 1 << "/" << merged_data.size() << "] "
                << "图像帧 #" << frame_count << " | 时间戳: " << data.timestamp
      ...
      imu_count++;
      std::cout << "[" << i + 1 << "/" << merged_data.size() << "] "
                << "IMU数据 #" << imu_count << " | 时间戳: " << data.timestamp
                << " | acc: [" << std::fixed << std::setprecision(3)
                << data.imu_data.accX << ", " << data.imu_data.accY << ", "
                << data.imu_data.accZ << "]"
                << " | gyro: [" << data.imu_data.gyroX << ", "
                << data.imu_data.gyroY << ", " << data.imu_data.gyroZ << "]"
                << std::endl;
    }
  }
#if OPENVINS_HAVE_OPENCV_HIGHGUI
  ...
  std::cout << "\n播放完成!" << std::endl;
  std::cout << "总共播放: " << frame_count << " 帧图像, " << imu_count
            << " 条IMU数据" << std::endl;
