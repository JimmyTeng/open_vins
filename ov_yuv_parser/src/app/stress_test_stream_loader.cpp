...
// 生成合成测试数据
static bool generateSyntheticData(const std::string& base_dir, int num_frames,
  ...
  for (int i = 0; i < num_frames; ++i) {
    info_file << "cam0.yuv " << i << " " << (base_ts + i * 33333) << " 1000 "
              << width << " " << height << "\n";
  }
  info_file.close();
...
int main(int argc, char* argv[]) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--frames" && i + 1 < argc) {
      ...
      cache_size =
          std::max(size_t(1), static_cast<size_t>(std::atoi(argv[++i])));
  ...
  std::string tmp_base =
      "/tmp/stress_test_stream_loader_" + std::to_string(getpid());
  ...
  for (int iter = 0; iter < num_iterations; ++iter) {
    ...
    while (loader.getNext(data)) {
      if (data.type == TimestampedData::FRAME) {
        frame_count++;
        if (data.frame_data.frame_data.size() !=
    ...
    if (frame_count != expected_frames || imu_count != expected_imu) {
      std::cerr << "迭代 " << iter << ": 数量异常 frame=" << frame_count << "/"
                << expected_frames << " imu=" << imu_count << "/"
                << expected_imu << std::endl;
      all_ok = false;
    }

    if ((iter + 1) % 5 == 0 || iter == 0) {
      std::cout << "  迭代 " << (iter + 1) << "/" << num_iterations << ": "
                << frame_count << " 帧, " << imu_count << " IMU, " << std::fixed
  ...
  if (rss_after > 0) {
    ...
    if (rss_before > 0 && delta > 5120) {
      std::cout << "警告: 内存增长 " << (delta / 1024.0) << " MB (可能存在泄漏)"
                << std::endl;
      all_ok = false;
    }
  }
  ...
  std::cout << "总计: " << total_frame_count << " 帧, " << total_imu_count
            << " IMU" << std::endl;
