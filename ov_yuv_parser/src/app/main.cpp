...
int main(int argc, char* argv[]) {
  ...
  std::cout << "=== YUV400 Parser with IMU (Time-synchronized) ==="
            << std::endl;
  ...
  if (!merged_data.empty()) {
    std::cout << "Time range: " << merged_data[0].timestamp << " to "
  ...
  for (size_t i = 0; i < merged_data.size(); ++i) {
    ...
    if (data.type == TimestampedData::FRAME) {
      frame_count++;
      std::cout << "[" << std::setw(10) << data.timestamp << "] FRAME #"
                << frame_count << " from " << data.frame_data.info.filename
                << " (frame " << data.frame_data.info.frame_count << ")"
                << std::endl;
      ...
      imu_count++;
      std::cout << "[" << std::setw(10) << data.timestamp << "] IMU #"
                << imu_count << ": ";
