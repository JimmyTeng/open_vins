...
int main(int argc, char* argv[]) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (arg == "--no-display" || arg == "--no-imshow" || arg == "-n") {
      ...
      std::cout
          << "VIO YUV Runner - 使用 ss_vio (ext) 接口处理 YUV 数据（流式加载）"
          << std::endl;
      std::cout << "用法: " << argv[0] << " [选项] [数据目录] [配置文件路径]"
                << std::endl;
      std::cout << "  --no-display, -n    不显示图像窗口" << std::endl;
      std::cout << "  --cache-size N      帧缓存大小，控制最大内存（默认 4）"
                << std::endl;
      return 0;
    } else {
  ...
  std::cout << "Found " << loader.scheduleSize()
            << " scheduled items (streaming mode, cache max " << max_frame_cache
            << " frames)" << std::endl;
